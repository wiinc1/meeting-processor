...
def test_notion_connection():
    ...
    # Try to get database IDs from .env file first
    notion_activities_db = os.getenv('NOTION_ACTIVITIES_DB')
    notion_tasks_db = os.getenv('NOTION_TASKS_DB')
    
    # If not in .env, get from config.yaml
    if not notion_activities_db or not notion_tasks_db:
        try:
            with open('config.yaml', 'r') as f:
                config = yaml.safe_load(f)
                
            notion_activities_db = notion_activities_db or config.get('notion_activities_db')
            notion_tasks_db = notion_tasks_db or config.get('notion_tasks_db')
        ...
        for db_name, db_id in [("Activities", notion_activities_db), ("Tasks", notion_tasks_db)]:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    db_id_no_hyphens = db_id.replace('-', '')
                    logger.info(f"  Database URL: https://notion.so/{db_id_no_hyphens}")
                else:
                    ...
                    # Print the URL for easy access
                    db_id_no_hyphens = db_id.replace('-', '')
                    logger.info(f"  Try accessing it at: https://notion.so/{db_id_no_hyphens}")
            except Exception as e:
