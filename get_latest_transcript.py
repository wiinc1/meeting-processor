...
and create an AI summary of the transcript.
"""
...
async def get_latest_transcript():
    """Get the latest meeting transcript and create a summary."""
    try:
        ...
        # Authenticate (this will require manual login)
        logger.info("🔐 Authenticating with Otter.ai...")
        auth_success = await scraper.authenticate()
        
        if not auth_success:
            logger.warning("⚠️ Authentication failed. You may need to manually authenticate.")
            logger.info("💡 Please complete authentication in the browser window if it opens.")
        ...
        # Create AI summary using a simple approach
...
async def main():
    ...
    result = await get_latest_transcript()
