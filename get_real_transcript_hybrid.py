...
using Selenium for authentication + Crawl4AI for infinite scroll scraping.
"""
...
async def get_real_latest_transcript_hybrid():
    """Get the REAL latest meeting transcript using Selenium auth + Crawl4AI scraping."""
    try:
        ...
        logger.info("=" * 70)
        logger.info("Using Selenium for auth + Crawl4AI for infinite scroll (best of both worlds!)")
        
        # Step 1: Use Selenium for authentication (more reliable)
        logger.info("🤖 Step 1: Using Selenium for authentication...")
        selenium_scraper = UnifiedOtterScraper(backend='selenium', headless=False)
        selenium_scraper.setup()
        ...
        auth_success = await selenium_scraper.authenticate()
        
        if not auth_success:
            logger.warning("⚠️ Authentication may have failed. Please try again.")
            return None
        
        logger.info("✅ Authentication successful with Selenium!")
        
        # Get recent meetings with Selenium (to get the list)
        logger.info("📋 Getting your recent meetings with Selenium...")
        meetings = await selenium_scraper.get_all_meetings()
        ...
        # Close Selenium scraper
        await selenium_scraper.close()
        logger.info("✅ Selenium scraper closed")
        
        # Step 2: Use Crawl4AI for fast scraping with infinite scroll
        logger.info("🚀 Step 2: Using Crawl4AI for fast transcript extraction with infinite scroll...")
        crawl4ai_scraper = UnifiedOtterScraper(backend='crawl4ai', headless=True)
        crawl4ai_scraper.setup()
        
        # Authenticate with Crawl4AI (should work since we're already logged in)
        logger.info("🔐 Authenticating with Crawl4AI...")
        auth_success = await crawl4ai_scraper.authenticate()
        
        if not auth_success:
            logger.warning("⚠️ Crawl4AI authentication failed, but continuing...")
        
        # Get detailed transcript with Crawl4AI
        logger.info("🔍 Getting detailed transcript with Crawl4AI...")
        meeting_details = await crawl4ai_scraper.get_meeting_details(latest_meeting['id'])
        
        if not meeting_details:
            logger.error("❌ Failed to get meeting details with Crawl4AI")
            return None
...
async def main():
    ...
    print("🎯 REAL Otter.ai Latest Meeting Transcript Extractor (Hybrid)")
    print("=" * 70)
    print("This script will:")
    print("1. Use Selenium for reliable authentication")
    print("2. Use Crawl4AI for fast transcript extraction with infinite scroll")
    ...
    result = await get_real_latest_transcript_hybrid()
